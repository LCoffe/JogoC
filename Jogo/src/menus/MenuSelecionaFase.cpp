...
namespace Menu {
	...
	void MenuSelecionaFase::criarBotoes() {
		incluiBotao(sf::Vector2f(100.0f, 400.0f), sf::Vector2f(100.0f, 50.0f), IDs::IDs::botaoFase01Jog1, sf::Vector2f(120.0f, 70.0f));
		incluiBotao(sf::Vector2f(100.0f, 450.0f), sf::Vector2f(100.0f, 50.0f), IDs::IDs::botaoFase01Jog2, sf::Vector2f(120.0f, 70.0f));
		incluiBotao(sf::Vector2f(100.0f, 500.0f), sf::Vector2f(100.0f, 50.0f), IDs::IDs::botaoFase02Jog1, sf::Vector2f(120.0f, 70.0f));
		incluiBotao(sf::Vector2f(100.0f, 550.0f), sf::Vector2f(100.0f, 50.0f), IDs::IDs::botaoFase02Jog2, sf::Vector2f(120.0f, 70.0f));
	...
	void MenuSelecionaFase::executar() {
		for (auto& it : botoes) {
			if (it->getSelecionado() && (IDs::IDs::botaoFase01Jog1 == it->getID() || IDs::IDs::botaoFase01Jog2 == it->getID())) {
				fundo->setTexture(pGG->incluirTextura(FUNDO_F1_PATH));
			}
			else if (it->getSelecionado() && (IDs::IDs::botaoFase02Jog1 == it->getID() || IDs::IDs::botaoFase02Jog2 == it->getID())) {
